...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        progressCancel.show().addClass('progressCancel').text('脳');
...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    var viewProgess = $('<button class="btn btn-default">鏌ョ湅鍒嗗潡涓婁紶杩涘害</button>');
...
FileProgress.prototype.setProgress = function(percentage, speed, chunk_size) {
    ...
    var progressbar = this.fileProgressWrapper.find('td .progress').find(
        '.progress-bar-info');
    if (this.fileProgressWrapper.find('.status').text() === '鍙栨秷涓婁紶') {
        return;
    }
    this.fileProgressWrapper.find('.status').text("宸蹭笂浼�: " + size + " 涓婁紶閫熷害锛� " +
        formatSpeed + "/s");
    ...
    if (chunk_size) {
        ...
        text = "鍧�" + current_uploading_chunk + "涓婁紶杩涘害" + current_chunk_percent +
            '%';
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (!isImg) {
        ...
        $(img).on('load', function() {
            ...
            linkWrapper.attr('href', url).attr('title', '鏌ョ湅鍘熷浘');
